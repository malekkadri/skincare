...
<section class="section">
    <div class="card" style="max-width:860px;margin:auto;">
        ...
        <form method="POST" action="{{ route('consultation.store') }}" class="grid grid-2" data-submit-once>@csrf
            <div><label>{{ __('consultation.first_name') }}</label><input name="first_name" value="{{ old('first_name') }}" required>@error('first_name')<div class="error">{{ $message }}</div>@enderror</div>
            <div><label>{{ __('consultation.last_name') }}</label><input name="last_name" value="{{ old('last_name') }}">@error('last_name')<div class="error">{{ $message }}</div>@enderror</div>
            <div><label>{{ __('consultation.phone') }}</label><input name="phone" value="{{ old('phone') }}" required>@error('phone')<div class="error">{{ $message }}</div>@enderror</div>
            <div><label>{{ __('consultation.email') }}</label><input type="email" name="email" value="{{ old('email') }}">@error('email')<div class="error">{{ $message }}</div>@enderror</div>
            ...
            <div style="grid-column:1/-1"><label>{{ __('consultation.main_concerns') }}</label><textarea name="main_concerns" required>{{ old('main_concerns') }}</textarea>@error('main_concerns')<div class="error">{{ $message }}</div>@enderror</div>
            ...
            <div style="grid-column:1/-1"><label><input type="checkbox" name="consent" value="1" required> {{ __('consultation.consent') }}</label>@error('consent')<div class="error">{{ $message }}</div>@enderror</div>
            <div style="grid-column:1/-1"><button class="btn" type="submit">{{ __('consultation.submit') }}</button></div>
        </form>
