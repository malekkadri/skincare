...
<section class="page-section section-tight-top">
    <div class="card consultation-shell centered-card reveal" style="--card-max:980px;">
        ...
        <form method="POST" action="{{ route('consultation.store') }}" class="grid grid-2" data-submit-once>
            ...
            <div class="form-field"><label for="first_name">{{ __('consultation.first_name') }}</label><input id="first_name" name="first_name" value="{{ old('first_name') }}" required aria-invalid="{{ $errors->has('first_name') ? 'true' : 'false' }}" aria-describedby="@error('first_name') first_name_error @enderror">@error('first_name')<div id="first_name_error" class="error">{{ $message }}</div>@enderror</div>
            <div class="form-field"><label for="last_name">{{ __('consultation.last_name') }}</label><input id="last_name" name="last_name" value="{{ old('last_name') }}" aria-invalid="{{ $errors->has('last_name') ? 'true' : 'false' }}" aria-describedby="last-name-help @error('last_name') last_name_error @enderror"><small id="last-name-help" class="field-help">Optional.</small>@error('last_name')<div id="last_name_error" class="error">{{ $message }}</div>@enderror</div>
            <div class="form-field"><label for="phone">{{ __('consultation.phone') }}</label><input id="phone" name="phone" value="{{ old('phone') }}" required aria-invalid="{{ $errors->has('phone') ? 'true' : 'false' }}" aria-describedby="@error('phone') phone_error @enderror">@error('phone')<div id="phone_error" class="error">{{ $message }}</div>@enderror</div>
            <div class="form-field"><label for="email">{{ __('consultation.email') }}</label><input id="email" type="email" name="email" value="{{ old('email') }}" aria-invalid="{{ $errors->has('email') ? 'true' : 'false' }}" aria-describedby="email-help @error('email') email_error @enderror"><small id="email-help" class="field-help">Optional, but helpful for follow-up.</small>@error('email')<div id="email_error" class="error">{{ $message }}</div>@enderror</div>
            <div class="form-field"><label for="preferred_language">{{ __('consultation.preferred_language') }}</label><select id="preferred_language" name="preferred_language" aria-invalid="{{ $errors->has('preferred_language') ? 'true' : 'false' }}" aria-describedby="@error('preferred_language') preferred_language_error @enderror"><option value="fr" @selected(old('preferred_language', app()->getLocale()) === 'fr')>FR</option><option value="en" @selected(old('preferred_language', app()->getLocale()) === 'en')>EN</option></select>@error('preferred_language')<div id="preferred_language_error" class="error">{{ $message }}</div>@enderror</div>
            <div class="form-field"><label for="age_range">{{ __('consultation.age_range') }}</label><input id="age_range" name="age_range" value="{{ old('age_range') }}" aria-describedby="age-help"><small id="age-help" class="field-help">Optional.</small></div>
            ...
            <div class="form-field form-span-full"><label for="main_concerns">{{ __('consultation.main_concerns') }}</label><textarea id="main_concerns" name="main_concerns" required aria-invalid="{{ $errors->has('main_concerns') ? 'true' : 'false' }}" aria-describedby="concerns-help @error('main_concerns') main_concerns_error @enderror">{{ old('main_concerns') }}</textarea><small id="concerns-help" class="field-help">Required. Share your top concerns, what you notice, and when they appear.</small>@error('main_concerns')<div id="main_concerns_error" class="error">{{ $message }}</div>@enderror</div>
