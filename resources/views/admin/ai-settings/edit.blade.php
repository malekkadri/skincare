...
<form method="POST" action="{{ route('admin.ai-settings.update') }}" class="card">@csrf @method('PUT')
    ...
    <div class="grid">
        ...
        <div><label>Provider</label><input name="ai_provider" value="{{ old('ai_provider', $settings->ai_provider ?? 'grok') }}">@error('ai_provider')<div class="error">{{ $message }}</div>@enderror</div>
        ...
        <div><label>Model</label><input name="ai_model" value="{{ old('ai_model', $settings->ai_model ?? 'grok-2-latest') }}"></div>
        <div><label>Base URL</label><input name="ai_base_url" value="{{ old('ai_base_url', $settings->ai_base_url) }}"></div>
